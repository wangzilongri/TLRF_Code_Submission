...
| `STEP2_R2C1_Train_GRF.py` | Trains TLRF with varied hyperparameter settings on the DGP data (HPC step); writes `R2C1_predictions.parquet` |
...
To regenerate from scratch (requires HPC for STEP2):

```bash
python STEP1_R2C1_DGP.py     # generates DGP data into data/
python STEP2_R2C1_Train_GRF.py  # trains GRF variants (HPC); writes R2C1_predictions.parquet
python STEP3_R2C1_Analyse_Results.py
```
